    ...
    */
    func insertEmoticon(emoticon: CZEmoticon) {
        ...
        if let pngPath = emoticon.pngPath {
            ...
            // 记录选中的范围
            let oldSelectdeRange = selectedRange
            // range: 替换的范围
            oldAttrString.replaceCharactersInRange(oldSelectdeRange, withAttributedString: attrString)
            // 赋值给textView的 attributedText
            attributedText = oldAttrString
            // 设置光标位置,在表情后面
            selectedRange = NSRange(location: oldSelectdeRange.location + 1, length: 0)
